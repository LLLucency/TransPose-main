...
class IMUSet:
    ...
    def _read(self):
        ...
        while self._is_reading:
            data += self._imu_socket.recv(1024).decode('ascii','ignore')
            strs = data.split(' ', num_float_one_frame)
            if len(strs) > num_float_one_frame:
                # print(np.array(strs[:-3]).reshape((21, 16)))  # full data
                d = np.array(strs[:36]).reshape((6, 16))  # first 6 imus
                tranc = int(len(self._quat_buffer) == self._buffer_len)  # tranc为0或1
